...
```bash
python cezar.py [-c | -a] [-e | -d | -j | -k]
```

### Błędy:

- Funkcja `decrypt` wymaga plaina, a nie powinna.
- Plik extra powinien mieć pierwsze litery tekstu jawnego, zamiast tego zapisuje decyrpted tekst dla każdego klucza.

Za zadanie dostałam **65/100pkt**.
...
- `extra.txt`: plik zawierający początek tekstu jawnego w przypadku kryptoanalizy z tekstem jawnym i zaszyfrowanym,
...
Program **łamiący szyfr** z pomocą tekstu jawnego czyta tekst zaszyfrowany, tekst pomocniczy i zapisuje znaleziony klucz i odszyfrowany tekst. Jeśli niemożliwe jest znalezienie klucza, zgłasza sygnał błędu.
...
**Program w żadnym wypadku nie ma prawa żądać istnienia plików niewymaganych dla danej opcji.**

Pliki, do których zapisujemy powinny być utworzone gdyby nie istniały.
...
Rozwiązanie powinno obejmować źródła programu, ew. wersja skompilowana jest opcjonalna (choć pożądana, gdy np. przygotowano program w Javie lub C# w środowisku Windows), oraz przykładowy tekst jawny, zaszyfrowany i poprawny klucz dla szyfru afinicznego. Przesyłać należy pakiet tar lub zip.
